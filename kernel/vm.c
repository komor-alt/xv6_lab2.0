...
/*
 * create a direct-map page table for the kernel.功能：初始化内核的页表，并建立内核空间的虚拟内存映射。
 */
void
kvminit()
{
  ...
  // // uart registers
  // kvmmap(UART0, UART0, PGSIZE, PTE_R | PTE_W);//在内核页表中添加一个虚拟地址到物理地址的映射。
...
walk(pagetable_t pagetable, uint64 va, int alloc)
{
  ...
  for(int level = 2; level > 0; level--) {
    ...
    if(*pte & PTE_V) {
      ...
      if(!alloc || (pagetable = (pde_t*)kalloc()) == 0) //如果alloc为1且页表项不存在，则分配一个新的页表页。
        return 0;
...
void
kvmmap(pagetable_t pgtbl,uint64 va, uint64 pa, uint64 sz, int perm)//在内核页表中添加一个虚拟地址到物理地址的映射。
{
...
uvminit(pagetable_t pagetable, uchar *src, uint sz)
{
  ...
  mem = kalloc();
  memset(mem, 0, PGSIZE);
  mappages(pagetable, 0, PGSIZE, (uint64)mem, PTE_W|PTE_R|PTE_X|PTE_U); //虚拟地址到物理地址的映射。
  memmove(mem, src, sz);
}

// Allocate PTEs and physical memory to grow process from oldsz to
// newsz, which need not be page aligned.  Returns new size or 0 on error.
uint64  //为用户进程分配物理内存并映射到用户页表。
...
int solve_pagetable(pagetable_t pagetable,int depth)
{
  for (int i = 0;i<512;i++)
  {
    ...
    if(page & PTE_V)
    {
      ...
      {
        uint64 child = PTE2PA(page); //equal to multply 4
